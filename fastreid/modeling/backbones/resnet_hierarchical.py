...
class ResNetHierarchical(nn.Module):
    """ResNet 支持返回多层级特征 (Layer3 + Layer4) 用于 Hierarchical OS-CA"""
    ...
    def forward(self, x, return_hierarchical=False):
        """
        Args:
            x: input images
            return_hierarchical: if True, return both layer3 and layer4 features
        """
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        ...
        feat_layer3 = x  # [B, 1024, H/16, W/16]
...
def build_resnet_hierarchical_backbone(cfg):
    """Create a ResNetHierarchical instance from config."""
    ...
    model = ResNetHierarchical(last_stride, bn_norm, with_ibn, with_se, with_nl, block,
